    ...
    withQAEnv {
        withMaven(maven: MAVEN_TOOL) {
            ...
            dir('its') {
                ...
                sh "mvn -Pit-ruling -Dsonar.runtimeVersion=${sqRuntimeVersion} -Dorchestrator.artifactory.apiKey=${env.ARTIFACTORY_PRIVATE_API_KEY} " +
...
def runPlugin(String sqRuntimeVersion) {
    withQAEnv {
        withMaven(maven: MAVEN_TOOL) {
            ...
            dir('its') {
                sh 'git submodule update --init --recursive'
                sh "mvn -Pit-plugin -Dsonar.runtimeVersion=${sqRuntimeVersion} -Dorchestrator.artifactory.apiKey=${env.ARTIFACTORY_PRIVATE_API_KEY} " +
                        "-Dorchestrator.configUrl=http://infra.internal.sonarsource.com/jenkins/orch-h2.properties -Dmaven.test.redirectTestOutputToFile=false clean verify -e -V"
            }
        }
    }
